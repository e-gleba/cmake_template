...
**Release:** click [▶ run release](https://github.com/e-gleba/cmake_template/actions/workflows/release.yml) — both version inputs are optional (empty = tag `v<project version>`, then patch-bump). A validate gate prints the previous tag and resolved release plan before any build starts; then it builds every platform, optionally pushes GHCR images, tags, attaches CPack/APK artifacts, and opens+merges a PR bumping `project(VERSION)` on main.
