/// @file web_main.cpp
/// Self-contained SDL3 + OpenGL + Dear ImGui demo.
///
/// Builds natively (OpenGL 3.3 core) and for the web via the emscripten
/// preset (WebGL2 / GLES3). Renders a fullscreen-triangle plasma - about
/// the cheapest animated shader possible: no textures, no buffers, vertex
/// positions derived from gl_VertexID. Shaders are inlined as constexpr
/// string views, so the web build needs zero asset preloading.
...
[[nodiscard]] bool load_gl_api(gl_api& api) noexcept
{
    ...
    const auto bind = [&ok](const char* name, auto& slot) noexcept {
        // Store through void** (gl3w-style): sidesteps both ISO C++ and
        // MSVC C4191 diagnostics on direct function-pointer casts.
